...
interface Props {
  ...
  busy?: boolean;
  onConfirm: (delta: number, mode: Mode, concept: string) => void | Promise<void>;
}
...
export default function AdjustCreditsDialog({ open, onOpenChange, currentCredits, busy, onConfirm }: Props) {
  ...
  useEffect(() => {
    if (open) { setMode("add"); setValue(""); setConcept(""); }
  }, [open]);

  const numeric = Number(value);
  const valid = value !== "" && !isNaN(numeric) && numeric >= 0 && concept.trim().length > 0;

  const preview = useMemo(() => {
    if (value === "" || isNaN(numeric)) return currentCredits;
    if (mode === "set") return numeric;
    if (mode === "add") return currentCredits + numeric;
    return Math.max(0, currentCredits - numeric);
  }, [mode, numeric, value, currentCredits]);

  const handleConfirm = async () => {
    if (!valid) return;
    await onConfirm(numeric, mode, concept.trim());
    onOpenChange(false);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3 py-2">
          ...
          <div className="space-y-1.5">
            <Label>
              Concepto / Justificación <span className="text-destructive">*</span>
            </Label>
            <Input
              value={concept}
              onChange={(e) => setConcept(e.target.value)}
              placeholder="Ej: Pago recibido, Ajuste de error…"
              maxLength={120}
            />
          </div>
