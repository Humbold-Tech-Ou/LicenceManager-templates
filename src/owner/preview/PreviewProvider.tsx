...
// ── Install mock client via installPreviewClient ──────────────────────────────
const mockClient = {
  ...
  auth: {
    ...
    // CRITICAL: call the callback immediately so OwnerAuthProvider exits "loading" state.
...
export function PreviewProvider({ children }: { children: ReactNode }) {
  ...
  useState(() => { installPreviewClient(mockClient); return null; });

  useEffect(() => {
    installPreviewClient(mockClient);
    document.title = "Vista previa — Mi Panel IPTV";
    return () => { installPreviewClient(null); };
  }, []);
